...
function App() {
  const [currentView, setCurrentView] = useState<'dashboard' | 'register' | 'list' | 'grades' | 'subjects' | 'attendance' | 'marks'>('dashboard');
  ...
  const fetchCounts = async () => {
    try {
      ...
      // Count attendance records from localStorage
      let totalAttendance = 0;
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key && key.startsWith('attendance_')) {
          const attendanceData = JSON.parse(localStorage.getItem(key) || '{}');
          totalAttendance += Object.keys(attendanceData).length;
        }
      }
      setAttendanceCount(totalAttendance);

      // Count marks from backend API
      try {
        const marksResponse = await fetch('http://localhost:5000/api/marks');
        if (marksResponse.ok) {
          const marksData = await marksResponse.json();
          setMarksCount(marksData.data?.length || marksData.length || 0);
        }
      } catch (error) {
        console.log('Marks API not available');
        setMarksCount(0);
      }
  ...
  return (
    <div className="App" style={{ minHeight: '100vh', display: 'flex', background: '#0f172a' }}>
      {/* Top Navigation Bar with Scoring */}
      <div style={{
        position: 'fixed',
        top: '0',
        left: '0',
        ...
        <div style={{ 
          ...
          <button
            ...
            style={{
              ...
              border: currentView === 'attendance' ? 'none' : '1px solid rgba(148, 163, 184, 0.2)',
              borderRadius: '12px',
              fontSize: '1rem',
              fontWeight: '600',
              cursor: 'pointer',
          ...
          >
            ...
            <div>
              <div style={{ marginBottom: '4px' }}>Manage Attendance</div>
              <div style={{
                fontSize: '0.85rem',
          ...
          <button
            ...
            style={{
              ...
              border: currentView === 'marks' ? 'none' : '1px solid rgba(148, 163, 184, 0.2)',
              borderRadius: '12px',
              fontSize: '1rem',
              fontWeight: '600',
              cursor: 'pointer',
          ...
          >
            ...
            <div>
              <div style={{ marginBottom: '4px' }}>Manage Marks</div>
              <div style={{
                fontSize: '0.85rem',
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: '1' }}>
          {currentView === 'dashboard' ? (
            ...
            <SubjectManagement /> 
          ) : currentView === 'attendance' ? <AttendanceManagement /> : currentView === 'marks' ? <MarksManagement /> : <StudentList />}
        </div>
      </div>
    </div>
  );
