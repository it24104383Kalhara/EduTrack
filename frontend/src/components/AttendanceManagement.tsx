...
const AttendanceManagement: React.FC = () => {
  ...
  const getAttendanceStatus = (studentId: number) => {
    if (!selectedGrade) return 'not-marked';
    const gradeAttendance = savedAttendanceRecords[selectedGrade.id];
    if (!gradeAttendance) return 'not-marked';
    
    // Get the most recent date's attendance
    const dates = Object.keys(gradeAttendance).sort().reverse();
    if (dates.length === 0) return 'not-marked';
    
    const latestDate = dates[0];
    return gradeAttendance[latestDate]?.[studentId] || 'not-marked';
  };
  if (loading) {
    return (
      <div style={{
