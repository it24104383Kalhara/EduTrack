...
export default function FindHospital({ onBack, onLogout }: FindHospitalProps) {
  ...
  const parseOverpassResults = (overpassData: any, cLat: number, cLon: number): Hospital[] => {
    ...
    return overpassData.elements
      ...
      .map((el: any) => {
        ...
        return {
          ...
          address: addrParts.length ? addrParts.join(', ') : 'Address not available',
  ...
  const fetchHospitalsByCoords = async (cLat: number, cLon: number) => {
    ...
    try {
      ...
      if (!overpassData) {
        setError('Medical facility search is temporarily unavailable. Please try again in a moment.');
        setHospitals([]);
      } else {
      ...
      setError('Could not fetch results. Please check your internet connection and try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchHospitals = async (locationQuery: string) => {
    ...
    try {
      ...
      const geoRes = await fetch(
        ...
        { headers: { 'Accept-Language': 'en', 'User-Agent': 'EduTrackHealth/1.0' } }
      );
    ...
    <div className="fh-root">
      ...
      <header className="fh-header">
        <div className="fh-header-inner">
          ...
          <div className="fh-title-group">
            ...
            <span className="fh-page-sub">Locate nearby medical facilities</span>
          </div>
      ...
      <main className="fh-main">
        ...
        <div className="fh-hero-card">
          ...
          <p className="fh-hero-sub">Use your GPS or type a location to discover nearby hospitals, clinics, and medical centres.</p>
          ...
          {gpsError && (
            <div className="fh-gps-error">⚠️ {gpsError}</div>
          )}

          {/* Search input */}
          <div className="fh-search-wrap">
            <span className="fh-search-icon"><SearchIcon /></span>
            <input
            ...
            />
            {query && (
              <button className="fh-clear-btn" onClick={() => setQuery('')} title="Clear">✕</button>
            )}
          </div>
        </div>
        ...
        <div className="fh-status-strip">
          {loading && (
            <>
              ...
              Searching for hospitals near <strong style={{color:'#059669',marginLeft:4}}>"{query}"</strong>…
            </>
          )}
          ...
          {!loading && searched && !error && hospitals.length === 0 && query && (
          ...
          <div className="fh-results">
            {!loading && !searched && (
              <div className="fh-prompt">
                ...
                <div className="fh-prompt-sub">Hospitals, clinics, and pharmacies in that area will appear here in real time.</div>
              </div>
            )}
            ...
            {!loading && searched && hospitals.length === 0 && (
              <div className="fh-empty">
                ...
                <div className="fh-empty-sub">Try a different or more specific location name.</div>
              </div>
            )}
              ...
              <div key={h.id} className="fh-card">
                ...
                <div className="fh-card-body">
                  <div className="fh-card-top">
                    <span className="fh-card-name" title={h.name}>{h.name}</span>
                    <span className={`fh-type-badge ${getTypeBadgeClass(h.type)}`}>
                      {getTypeLabel(h.type)}
                    </span>
                  </div>

                  <div className="fh-card-address">
                    📍 {h.address}
                  </div>
