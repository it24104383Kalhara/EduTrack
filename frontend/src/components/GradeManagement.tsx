...
const GradeManagement: React.FC = () => {
  ...
  const openBulkEnrollment = async (gradeId: number) => {
    try {
      ...
      const isAL = gradeNum === '12' || gradeNum === '13';
      const filtered = allSubjects.filter(s => {
        const matchesGrade = (Array.isArray(s.grades) ? s.grades : [s.grades]).includes(gradeNum || '');
        if (!matchesGrade || !s.category) return false;

        // For A/L (Grades 12 & 13) classes, only show subjects matching the class stream
        if (isAL && s.stream) {
          const subjectStreams = Array.isArray(s.stream) ? s.stream : [s.stream];
          const gradePart = gradeObj?.grade_part.toLowerCase() || '';
