            ...
            <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-100' : 'bg-transparent'}`}>
                ...
                {menuOpen && (
                    <div className="lg:hidden bg-white border-t border-gray-100 px-6 py-4 space-y-1 shadow-lg">
                        {MODULES.map(m => (
                            <Link key={m.id} to={m.href} className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-700 hover:text-[#633194] ${m.bg} transition-all`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
                        ...
                        <div>
                            ...
                            <ul className="space-y-2 text-sm">
                                {MODULES.map(m => <li key={m.id}><Link to={m.href} className="hover:text-white transition-colors">{m.label}</Link></li>)}
                            </ul>
                        </div>
                        ...
                        <div>
                            ...
                            <ul className="space-y-2 text-sm">
                                {['Dashboard', 'Attendance', 'Reports', 'Notifications', 'Analytics'].map(l => (
                                    <li key={l}><Link to="/dashboard" className="hover:text-white transition-colors">{l}</Link></li>
                                ))}
                            </ul>
                        </div>
